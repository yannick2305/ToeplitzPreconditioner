...
% ==== Parameters ====
    ...
    num_lambda = 600;    % Number of plotting points (50-300)
...
% === Spline to get continuous GBZ ===
    chord = resampleCurveByAngle(openLimit_sorted, num_lambda*10);
...
% ==== Quasi Similarity Transformed Toeplitz matrix ====
    ...
    hold on;
    plot(real(eigT), imag(eigT),         'gx', 'MarkerSize', 8, 'LineWidth', 1.5);
...
function [lambdaL, lambdaR] = open_limit(a)
    ...
    min_step = 1e-6;   % geometric: stop bisecting here (≪ tol/|F'|)
